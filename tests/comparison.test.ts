...
async function runComparisonTests() {
  ...
  const fencedEvents: ComparisonEvent[] = [];
  const tracker = new GenerationTracker('session-fenced', 1);
  ...
  const oldToolPromise = (async () => {
    try {
      const res = await searchProducts(
        { category: 'laptops', max_price: 1000, delayMs: 300 },
        { signal: toolOp1.abortController.signal }
      );
  ...
  const newToolRes = await searchProducts(
    { category: 'laptops', max_price: 800, min_ram_gb: 16, delayMs: 50 },
    { signal: toolOp2.abortController.signal }
  );
  ...
  fencedEvents.forEach((e) =>
    console.log(`  - [${e.event_type}] Gen: ${e.generation_id ?? 'N/A'}, Op: ${e.operation_id ?? 'N/A'}, Accepted: ${e.result_accepted}, DiscardReason: ${e.discard_reason ?? 'none'}`)
  );

  const fencedOldResultAccepted = fencedEvents.some(
    (e) => e.operation_id === toolOp1.operation_id && e.event_type === 'tool_result_accepted'
  );
  const fencedNewResultAccepted = fencedEvents.some(
    (e) => e.operation_id === toolOp2.operation_id && e.event_type === 'tool_result_accepted'
  );
  const fencedOldResultDiscarded = fencedEvents.some(
    (e) => e.operation_id === toolOp1.operation_id && (e.event_type === 'tool_result_discarded' || e.event_type === 'tool_cancelled')
  );
  ...
  const unfencedEvents: ComparisonEvent[] = [];
  ...
  const baselineOldToolPromise = (async () => {
    ...
    const res = await searchProducts(
      { category: 'laptops', max_price: 1000, delayMs: 0 }
    );
  ...
  const baselineNewRes = await searchProducts({
    category: 'laptops',
    max_price: 800,
    min_ram_gb: 16,
    delayMs: 50,
  ...
  unfencedEvents.forEach((e) =>
    console.log(`  - [${e.event_type}] Gen: ${e.generation_id ?? 'N/A'}, Op: ${e.operation_id ?? 'N/A'}, Accepted: ${e.result_accepted}`)
  );

  const unfencedOldResultAccepted = unfencedEvents.some(
    (e) => e.operation_id === 'baseline-op-1' && e.event_type === 'tool_result_accepted'
  );
  const unfencedNewResultAccepted = unfencedEvents.some(
    (e) => e.operation_id === 'baseline-op-2' && e.event_type === 'tool_result_accepted'
  );
