...
async function runTests() {
    ...
    const res1 = await (0, shopping_1.searchProducts)({ category: 'headphones', max_price: 200, delayMs: 0 });
    console.log(`Matched: ${res1.products.length} products`);
    res1.products.forEach(p => console.log(` - ${p.name} ($${p.price})`));
    const test1Pass = res1.products.every(p => p.category === 'headphones' && p.price <= 200);
    console.log(`Test 1 Passed: ${test1Pass}\n`);
    // TEST 2: Bose headphones
    console.log('Test 2: Find Bose headphones');
    const res2 = await (0, shopping_1.searchProducts)({ category: 'headphones', brand: 'Bose', delayMs: 0 });
    console.log(`Matched: ${res2.products.length} products`);
    res2.products.forEach(p => console.log(` - ${p.name} ($${p.price})`));
    const test2Pass = res2.products.every(p => p.category === 'headphones' && p.brand === 'Bose');
    console.log(`Test 2 Passed: ${test2Pass}\n`);
    // TEST 3: Laptops under $1000
    console.log('Test 3: Find laptops under $1000');
    const res3 = await (0, shopping_1.searchProducts)({ category: 'laptops', max_price: 1000, delayMs: 0 });
    console.log(`Matched: ${res3.products.length} products`);
    res3.products.forEach(p => console.log(` - ${p.name} ($${p.price}, ${p.ram_gb}GB RAM)`));
    const test3Pass = res3.products.every(p => p.category === 'laptops' && p.price <= 1000);
    ...
    const res4 = await (0, shopping_1.searchProducts)({ category: 'laptops', max_price: 800, min_ram_gb: 16, delayMs: 0 });
    console.log(`Matched: ${res4.products.length} products`);
    res4.products.forEach(p => console.log(` - ${p.name} ($${p.price}, ${p.ram_gb}GB RAM)`));
    const test4Pass = res4.products.every(p => p.category === 'laptops' && p.price <= 800 && (p.ram_gb ?? 0) >= 16);
    ...
    await (0, shopping_1.searchProducts)({ category: 'headphones', delayMs: 1000 });
