...
export interface TrialRecord {
  trial_id: number;
  implementation: 'fenced' | 'unfenced';
  started_at: number;
  interruption_at: number;
  old_generation: number | null;
  new_generation: number | null;
  old_operation_id: string;
  new_operation_id: string;
  cancellation_requested: boolean;
  cancellation_result: 'succeeded' | 'lost_race_completed' | 'not_requested';
  old_operation_completed: boolean;
  old_result_accepted: boolean;
  old_result_discarded: boolean;
  discard_reason?: string;
  new_result_accepted: boolean;
  recovered: boolean;
  interruption_to_invalidation_ms: number;
  new_response_latency_ms: number;
}
...
export interface SummaryStats {
  ...
  stale_leak_rate_pct: number;
  cancellation_requested_count: number;
  cancellation_succeeded_count: number;
  cancellation_success_rate_pct: number;
  fence_catches: number;
  fence_catch_rate_pct: number;
  recovery_successes: number;
  recovery_success_rate_pct: number;
  interruption_to_invalidation_median_ms: number;
  interruption_to_invalidation_p95_ms: number;
  new_response_latency_median_ms: number;
  new_response_latency_p95_ms: number;
}
...
export async function runStressTest(totalTrials = 100) {
  console.log(`\n================================================================`);
  console.log(`STAGE 10: AUTOMATED STRESS TEST (${totalTrials} TRIALS)`);
  console.log(`================================================================\n`);

  const fencedRecords: TrialRecord[] = [];
  const unfencedRecords: TrialRecord[] = [];
  ...
  for (let i = 1; i <= totalTrials; i++) {
    const tracker = new GenerationTracker(`fenced-session-${i}`, 1);
    ...
    let cancellationResult: 'succeeded' | 'lost_race_completed' | 'not_requested' = 'not_requested';
      ...
      try {
        if (cancellationWins) {
          ...
          await searchProducts(
            { category: 'laptops', max_price: 1000, delayMs: 20 },
            { signal: oldOp.abortController.signal }
          );
        } else {
          // Tool loses cancellation race and completes anyway (ignores abort / completes late)
          await searchProducts({ category: 'laptops', max_price: 1000, delayMs: 10 });
        }
    ...
    await searchProducts({ category: 'laptops', max_price: 800, min_ram_gb: 16, delayMs: 5 });
  ...
  // ==============================================================
  function computeSummary(records: TrialRecord[], impl: 'fenced' | 'unfenced'): SummaryStats {
    ...
    const fenceCatchRate =
      completedAfterCancel.length > 0 ? (fenceCatches / completedAfterCancel.length) * 100 : 0;
  ...
  const rapidTracker = new GenerationTracker('rapid-session', 1);
  ...
  const concurrentTracker = new GenerationTracker('concurrent-session', 10);
  ...
  const concurrentPass =
    !concurrentTracker.isCurrent(oldLlm) &&
  ...
  fs.writeFileSync(
    path.join(resultsDir, 'stage10-results.json'),
